...
import {MasAxiosResponseType, ShowMessageType, type TransMasFunType, TransFunType} from "./ToResult/types";
 ...
 */
export default class AxiosTo<T extends AxiosResponse = MasAxiosResponseType> {
  // 消息展示方法
  readonly showMessage: ShowMessageType;
  // 解析返回值方法
  ...
  async async<R extends T>(promise: Promise<R>, silence = false) {
    ...
    return promise.then(data => new ToResult<R, TransMasFunType<R>>(data, undefined, this.#getInfoFun))
      .catch(e => {
        if (!silence) {
          let message: string;
          if (typeof e === 'string')
        ...
        return new ToResult<R, TransMasFunType<R>>(undefined, e, this.#getInfoFun);
      });
  }
}