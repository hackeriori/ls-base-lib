/**
 * 保存canvas指定区域为Data URL(base64)
 * 如果不需要指定区域，那么直接用canvas.toDataURL方法，因为该方法会通过临时canvas中转
 * @param canvas 画布元素
 * @param x 左上角x坐标
 * @param y 左上角y坐标
 * @param width 宽度
 ...
 */
export function imgGetCanvasRect(canvas: HTMLCanvasElement, x: number, y: number, width: number, height: number, exportType?: string, exportQuality?: number): string | undefined {
  ...
  if (ctx) {
    ...
    if (tempCtx) {
      ...
      return tempCanvas.toDataURL(exportType, exportQuality)
    }
  }
}
...
/**
 * 重新调整图片大小，一般用于压缩图片，返回Data URL
 * @param img HTMLImageElement
 * @param sortSide 短边长度
 */
export function imgResize(img: HTMLImageElement, sortSide: number) {
